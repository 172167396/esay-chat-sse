...
(function ($, window, document) {
    ...
    Plugin.prototype = {
        _init: function (options) {
            ...
            btnLeft = options?.left ?? $(btn).offset().left;
            btnTop = options?.top ?? $(btn).offset().top + $(btn).outerHeight() + 5;
            scale = options?.scale ?? 1.0;

            $('#emoji_container_' + ix).css({
                'top': btnTop.endsWith("px") ? btnTop : btnTop + 'px',
                'left': btnLeft.endsWith("px") ? btnLeft : btnLeft + 'px',
            ...
            let $emojiIcon = $('#emoji_container_' + ix + ' .emoji_icons a');
            $emojiIcon.mouseenter(function () {
